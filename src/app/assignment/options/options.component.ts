...
export class OptionsComponent implements OnInit {
  ...
  setActive(element: EventTarget) {
    this.removeActives();
    let parentElement = (<HTMLInputElement>element).parentElement;
    this.renderer.setElementClass(parentElement.firstElementChild, 'active', true);
  }

  removeActives() {
    for (let element of this.optionElms['_results']) {
      this.renderer.setElementClass(element.nativeElement.parentElement.firstElementChild, 'active', false);
    }
  }
